...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean updateData(ModelData model) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(COL2, model.getTitle());
        contentValues.put(COL3, model.getDate());
        contentValues.put(COL4, model.getTime());
        Log.d(TAG, "updateData: updating " + model.getTitle() + " to " + TABLE_NAME);
        long result = db.update(TABLE_NAME, contentValues, COL1 + "=?", new String[]{String.valueOf(model.getId())});
        db.close();
        return result != -1;
    }
